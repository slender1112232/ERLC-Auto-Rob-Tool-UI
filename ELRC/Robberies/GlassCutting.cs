    ...
    public static class GlassCutting
    {
        ...
        public static void StartProcess()
        {
            ...
            try
            {
                while (true)
                {
                    // Kiểm tra nếu Roblox mất focus (do alt-tab)
                    if (!Roblox.IsRobloxFocused())
                    {
                        Console.WriteLine("i ~ Roblox lost focus, stopping GlassCutting...");
                        break;
                    }
            ...
            {
                // Thả chuột bằng cách đặt về trung tâm màn hình
                Mouse.SetMousePos(Screen.ScreenWidth / 2, Screen.ScreenHeight / 2);
                Console.WriteLine("i ~ GlassCutting stopped, mouse focus released.");
            }
        }
    }
}